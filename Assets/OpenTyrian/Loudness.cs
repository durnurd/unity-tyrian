...
public static class LoudnessC
{
    ...
    private static uint[] channel_len = new uint[SFX_CHANNELS];
    ...
    public static bool init_audio()
    {
        ...
        AudioClip = AudioClip.Create("", freq * 2, 1, freq, true, audio_cb);
    ...
    private static void audio_cb(float[] feedme)
    {
        ...
        if (!samples_disabled)
        {
            ...
            for (int ch = 0; ch < SFX_CHANNELS; ch++)
            {
                ...
                {
                    int clip = (int)feedme[smp] + (int)(channel_pos[ch][smp + channel_pos_offset[ch]] * volume);
