...
public static class ShotsC
{
    public class PlayerShotDataType
    {
        ...
        public JE_word shotGr, shotAni, shotAniMax;
        public byte shotDmg;
        public JE_byte shotBlastFilter, chainReaction, playerNumber, aimAtEnemy, aimDelay, aimDelayMax;
    }

    public const int MAX_PWEAPON = 81;

    // I'm pretty sure the last extra entry is never used.
    public static PlayerShotDataType[] playerShotData = EmptyArray<PlayerShotDataType>(MAX_PWEAPON + 1); /* [1..MaxPWeapon+1] */
    public static JE_byte[] shotAvail = new JE_byte[MAX_PWEAPON]; /* [1..MaxPWeapon] */   /*0:Avail 1-255:Duration left*/
        ...
        for (int z = 0; z < MAX_PWEAPON; z++)
        {
            if (shotAvail[z] != 0)
            {
                ...
                if (z != MAX_PWEAPON - 1)
                {
                    ...
                    JE_word anim_frame = (JE_word)(shot.shotGr + shot.shotAni);
    ...
    private static readonly JE_word[] linkMultiGr /* [0..16] */ =
        {77,221,183,301,1,282,164,202,58,201,163,281,39,300,182,220,77};
    private static readonly JE_word[] linkSonicGr /* [0..16] */ =
    ...
    public static void player_shot_set_direction(JE_integer shot_id, uint weapon_id, JE_real direction)
    {
        ...
        shot.shotXM = (JE_integer)(-Round(Sin(direction) * shot.shotYM));
        shot.shotYM = (JE_integer)(-Round(Cos(direction) * shot.shotYM));
        ...
        switch (weapon_id)
        {
            ...
            case 32:
            case 10:
                rounded_dir = (int)(Round(direction * (16 / (2 * PI))));  /*16 directions*/
    ...
    public static bool player_shot_move_and_draw(
            ...
            out JE_integer out_shot_damage, out JE_byte out_blast_filter,
            out JE_byte out_chain, out JE_byte out_playerNum,
            out JE_word out_special_radiusw, out JE_word out_special_radiush)
    {
        ...
        out_chain = 0;
        out_playerNum = 0;
        out_special_radiusw = 0;
        out_special_radiush = 0;
        ...
        if (shot_id != MAX_PWEAPON - 1)
        {
            ...
            JE_word sprite_frame = (JE_word)(shot.shotGr + shot.shotAni);
            if (++shot.shotAni == shot.shotAniMax)
                shot.shotAni = 0;
            ...
            if (out_is_special)
            {
                ...
                var frame = sprite(OPTION_SHAPES, sprite_frame - 60001);
                out_special_radiusw = (JE_word)(frame.width / 2);
                out_special_radiush = (JE_word)(frame.height / 2);
            }
            else
            {
                if (sprite_frame > 1000)
                {
                    JE_doSP(out_shotx + 1 + 6, out_shoty + 6, 5, 3, (byte)((sprite_frame / 1000) << 4));
                    sprite_frame = (JE_word)(sprite_frame % 1000);
                }
                if (sprite_frame > 500)
                {
    ...
    public static JE_integer player_shot_create(uint portNum, uint bay_i, int PX, int PY, JE_word mouseX, JE_word mouseY, JE_word wpNum, JE_byte playerNum)
    {
        ...
        JE_WeaponType weapon = weapons[wpNum];
        ...
        JE_integer shot_id = MAX_PWEAPON;
        /*Rot*/
        for (int multi_i = 1; multi_i <= weapon.multi; multi_i++)
        {
            ...
            else
            {
                ...
                if (circsize > 19)
                {
                    JE_byte circsize_mod20 = (JE_byte)(circsize % 20);
                    shot.shotCirSizeX = circsize_mod20;
                    shot.shotDevX = (JE_integer)(circsize_mod20 >> 1);

                    circsize = (JE_byte)(circsize / 20);
                    shot.shotCirSizeY = circsize;
                    shot.shotDevY = (JE_integer)(circsize >> 1);
                }
                else
                {
                    shot.shotCirSizeX = circsize;
                    shot.shotCirSizeY = circsize;
                    shot.shotDevX = (JE_integer)(circsize >> 1);
                    shot.shotDevY = (JE_integer)(circsize >> 1);
                }
                shot.shotDirX = 1;
                shot.shotDirY = -1;
            ...
            {
                shot.chainReaction = (JE_byte)(weapon.attack[shotMultiPos[bay_i] - 1] - 100);
                shot.shotDmg = 1;
            }
            else
            {
            ...
            shot.shotX = (JE_integer)(PX + weapon.bx[shotMultiPos[bay_i] - 1]);

            shot.shotY = (JE_integer)(PY + tmp_by);
            shot.shotYC = (JE_integer)(-weapon.acceleration);
            ...
            if (del > 100 && del < 120)
                shot.shotAniMax = (JE_word)(del - 100 + 1);
            else
                shot.shotAniMax = (JE_word)(weapon.weapani + 1);

            if (del == 99 || del == 98)
            {
                tmp_by = (JE_integer)(PX - mouseX);
                if (tmp_by < -5)
                    tmp_by = -5;
                else if (tmp_by > 5)
                    tmp_by = 5;
            ...
            if (del == 99 || del == 100)
            {
                tmp_by = (JE_integer)(PY - mouseY - weapon.sy[shotMultiPos[bay_i] - 1]);
                if (tmp_by < -4)
                    tmp_by = -4;
                else if (tmp_by > 4)
                    tmp_by = 4;
            ...
            {
                shot.shotYM = (JE_integer)(-weapon.sy[shotMultiPos[bay_i] - 1]);
            }
            ...
            if (weapon.aim > 5)  /*Guided Shot*/
            {
                ...
                for (x = 0; x < 100; x++)
                {
                    ...
                    {
                        y = (JE_word)(Abs(enemy[x].ex - shot.shotX) + Abs(enemy[x].ey - shot.shotY));
                        if (y < best_dist)
                        {
                            best_dist = y;
                            closest_enemy = (JE_byte)(x + 1);
                        }
                    }
                }
                shot.aimAtEnemy = closest_enemy;
                shot.aimDelay = 5;
                shot.aimDelayMax = (JE_byte)(weapon.aim - 5);
            }
            else
            {
                shot.aimAtEnemy = 0;
